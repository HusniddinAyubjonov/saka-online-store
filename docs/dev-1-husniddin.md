...
| `widgets/catalog-filters` | сайдбар: группы `Checkbox` (тип ткани, состав), выбор цвета (`ColorPicker` от Разработчика 2), диапазон цены, кнопка «Сбросить». На планшете/мобайле — прячется в модалку/выезжающую панель (макеты `505-7933`, `505-8188`) |
| `widgets/catalog-grid` | сетка `ProductCard` (desktop 3–4 в ряд → планшет 2 → мобайл 1) |

`Breadcrumbs` берёшь у Разработчика 2 (или временно сверстай простой список ссылок).
...
| Виджет | Что внутри |
|---|---|
| `widgets/cart-summary` | список добавленных тканей (мини-`ProductCard`: фото + название + `QuantityStepper` от Разработчика 2 + цена + иконка удалить), справа/снизу — блок «Итого» с суммой и `Button` «Оформить заказ» |
...
Это по сути формы + список — просто. `QuantityStepper` берёшь у Разработчика 2.
...
| `widgets/account-profile` | фото пользователя (кружок), поля `Input` (имя, телефон, e-mail), кнопка «Сохранить», карточки со статистикой (`Stat` от Разработчика 2) |
