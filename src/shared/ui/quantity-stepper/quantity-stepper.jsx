...
export const QuantityStepper = ({
  ...
  return (
    <div className={[styles.stepper, styles[size], className].filter(Boolean).join(" ")}>
      <button type="button" aria-label="Убрать" onClick={() => set(value - 1)} disabled={value <= min}>
        <Icon name="minus" size={size === "sm" ? 12 : 16} />
      </button>
      <span className={styles.value}>{value}</span>
      <button type="button" aria-label="Добавить" onClick={() => set(value + 1)} disabled={value >= max}>
